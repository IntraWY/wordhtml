...
All 17 items from `wordhtml-feature-proposal.html` are shipped (A1–A3, B1–B9, C1–C4). **938 unit tests pass; lint + build clean; e2e 34/34.**

**v0.2.23 — Word-style table handles: drag row height, drag the table's LEFT edge, move/select handle, and ruler column markers.** User report: "ลากขอบซ้าย-ขวาของตารางไม่ได้ เพราะติด ruler" + "อยากให้ตารางเหมือน Word". Root cause was an **engine limit**, not a bug: prosemirror-tables' column resize only drags *between* columns + the last column's right edge — it can't drag the table's **left edge**, **row heights**, or **move** a table. Fix adds custom handles for exactly those, reusing prosemirror's built-in column resize for what it already does. Three phases, all shipped + live-verified in Chrome (no console errors):
- **Phase 1 — row-height drag.** `src/lib/tiptap/tableRowResize.ts` (`TableRowResize` extension) hit-tests a ~10px band at each row's bottom border (shows `row-resize` cursor via `.wh-row-resize-hot`), live-previews the `<tr>` height via rAF, and commits the existing `rowHeight` attr (tableProperties) in ONE transaction on mouseup (single undo, **no export change**). Pure math in `rowResizeMath.ts` (`computeRowHeight`, clamp/round, `MIN_ROW_PX`). Dialog copy in `TablePropertiesDialog.tsx` updated (rows *are* draggable now).
- **Phase 2 — `TableHandleOverlay.tsx`** (mounted in `EditorShell` inside `#editor-main`, PageChromeLayer pattern): a **left-edge grip** (Word semantics = right edge fixed, table shifts right + first column narrows: new `tableIndent` attr → `margin-left` + `colwidth` reduction) and a **move/select handle** (click = select whole table via `selectWholeTable`; drag = discrete reorder among sibling blocks — true free-float is out of scope in the `(block|pageBreak)+` model). Right edge + inter-column stay with prosemirror's plugin; the overlay reads `columnResizingPluginKey` to suppress its handles during a plugin drag. Because `#editor-main` is a stacking sibling BELOW the sticky ruler, a table under the ruler is hidden (Word-correct) and no handle paints over the ruler — the "ติด ruler" issue is gone.
- **Phase 3 — `RulerColumnMarkers.tsx`**: Word-style column-boundary markers on the horizontal ruler when the caret is in a table; drag one to resize that boundary (grows left col, shrinks right, total fixed). Self-contained overlay measured from live cell rects (1:1 page-px in the ruler track) → **no changes to the fragile `Ruler.tsx`/`useRulerDrag.ts`**.
- **Write-back + pure helpers:** `tableCommands.ts` (`applyTableLeftEdge`, `setColumnWidths`, `moveTableToIndex`; shared `writeColwidths` mirrors prosemirror's per-cell `colwidth` so `bakeTableColumns` export is unchanged), `tableColwidth.ts` (`readColumnWidths`, `resizeFromLeftEdge`, `resizeColumnBoundary`), `tableMoveMath.ts` (`targetIndexFromY`). `tableIndent` joins `MANAGED_TABLE_STYLE_PROPS`/`applyManagedTableStyle` (additive `margin-left`, never clobbers plugin width) and is kept on export (`margin-left` in cleaner KEEP set + not stripped by `bakeTableColumns.setTableWidth`, locked by a new test).
- **Verified:** left-edge drag → margin-left 0→40px, first col 216→176, right edge fixed; row drag 57→87px; move-handle click selects 6/6 cells; ruler marker drag → col +40/−40, total fixed, `colwidth` persisted. **938 unit tests** (908 + 30 new across `rowResizeMath`/`tableColwidth`/`tableMoveMath`/`bakeTableColumns`), lint + build clean. Known v1 limits (by design): overlay right-edge delegated to prosemirror; free-floating tables out of scope; split-continuation guard deferred.

**v0.2.22 — Fix: Table Properties cell margins/spacing now show in the EDITOR, not just Preview.** Bug from v0.2.21: after setting cell margins / cell spacing, the **ดูตัวอย่าง (Preview)** toggle showed the spacing but the live editor did **not** ("ระยะห่าง/ช่องไฟต่างกัน"). Root cause (verified against `@tiptap/extension-table` dist, not the browser): the v0.2.21 `cellMargins`/`cellSpacing` table attributes emit their CSS **only through `renderHTML`** (i.e. only during `getHTML()` serialization). Preview injects `getHTML()` output into `.prose-editor` → the `td,th { padding: var(--wh-pad-*) }` rule applies → spacing shows. But because `Table.configure({ resizable: true })`, the **live editor** renders tables through prosemirror-tables' `TableView` node view (supplied to the `columnResizing` plugin; `addNodeView` returns `null` when resizable), which builds its own `<table>` and applies inline CSS **only from `node.attrs.style`** (`if (node.attrs.style) this.table.style.cssText = …`) — never running our global-attribute `renderHTML`. We never create an attribute literally named `style`, so the editor ignored the margins/spacing. (`rowHeight` is on `<tr>`, rendered normally in both, so it already matched — consistent with the user reporting padding/gaps, not row height.) Worse, `TableView.update()` doesn't re-apply `cssText` (only the constructor does), so a bare `style` attr would only show after remount. Fix (2 files, no attribute/command/cleaner/export change so Preview + downloads stay byte-identical): new exported pure helper **`applyManagedTableStyle(table, attrs)`** in `tableProperties.ts` writes `--wh-pad-*` + `border-collapse:separate;border-spacing` onto the live `<table>` via `setProperty` (additive — never clobbers the column width `updateColumns` sets; clears the managed set first so toggling a prop off removes only it), mirroring the `renderHTML` output; new **`StyleAwareTableView extends TableView`** (TableView is exported) calls it on **both** construct and `update`, wired via `Table.configure({ View: StyleAwareTableView })` in `VisualEditor.tsx` (column resizing unchanged; horizontal lines still non-draggable). Net: editor == Preview. Locked by 5 new tests in `tableProperties.test.ts` (pure `applyManagedTableStyle`: write/omit/toggle-off/preserve-width + a live-editor integration test asserting `editor.view.dom` table gets `--wh-pad-t`/`border-spacing` and loses border-spacing on toggle-off). **908 unit tests pass**; build + lint clean.
