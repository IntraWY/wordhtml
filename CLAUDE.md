...
### Pagination
Holistic reflow via `repaginate.ts` (`computePageBreaks` fill-to-limit). Over-tall paragraphs soft-split (`data-soft-split`, `findParagraphSegments`). Ghost pages stripped by `normalizeIncomingHtml.ts`. All five export formats re-join soft-split pieces via `stripPaginationWrappers.ts`. Auto table-split (`tableReflow.ts`) only **repeats** the first row as a header on continuation pieces when that row is a real header — `isHeaderRow` requires every cell to be a `<th>`/`tableHeader`. This is intentionally conservative: a Thai-gov table whose first row is `<td>` won't get its first row repeated on auto-split (the manual `tableSplit.ts` command repeats row 0 unconditionally — that asymmetry is a UX choice, not data loss; the auto path is always lossless, every original row appears exactly once). On re-join, `stripPaginationWrappers.absorbTablePiece` always appends absorbed content rows into a `<tbody>` (creating one if the target only has a `<thead>`) so no data row ever lands in `<thead>`.
