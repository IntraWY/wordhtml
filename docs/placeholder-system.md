...
| **Control block** | `{{#if}}` `{{#unless}}` `{{#each}}` | เงื่อนไข/วนซ้ำตอน export | `{{#if vip}}…{{/if}}` |

### Control blocks (GAP 08 — เงื่อนไข + วนซ้ำ)

โมดูล `conditionalBlocks.ts` — pure, ทำงานตอน export เท่านั้น (resolve **ก่อน** `{{var}}`):

| Syntax | ความหมาย |
|--------|----------|
| `{{#if name}}…{{/if}}` | แสดง body เมื่อ `name` truthy |
| `{{#if name}}…{{else}}…{{/if}}` | สลับ branch |
| `{{#unless name}}…{{/unless}}` | ตรงข้ามกับ `if` (รองรับ `{{else}}` ด้วย) |
| `{{#each list}}…{{this}}…{{/each}}` | วนซ้ำ list variable |

- **Truthiness:** ค่า resolved (dataRow ก่อน แล้ว default) ที่เป็น string ไม่ว่าง-ไม่ใช่ whitespace = TRUE. ว่าง/ขาด/whitespace = FALSE. หมายเหตุ: `"0"`/`"false"` ถือเป็น TRUE (non-empty) — ถ้าต้องการ false ให้เก็บเป็น string ว่าง.
- **Loop items:** วน `listValues` (หรือ fallback: split `value` ด้วย `delimiter`). `{{this}}` = item ปัจจุบัน; ถ้า item เป็น JSON object string รองรับ `{{this.field}}`. รองรับ filter: `{{this|upper}}`, `{{this.field|baht}}`. list ว่าง/ขาด → render ว่าง.
- **Nesting:** parser แบบ depth-counter จับคู่ tag ถูกต้อง (if ซ้อน if, each ซ้อนได้).
- **Safety:** tag ไม่สมดุล (`{{#if}}` ไม่มี `{{/if}}`) ปล่อยข้อความเดิม ไม่ throw, ไม่ loop ค้าง.
- **Health check:** `stripControlBlocksForHealth()` ตัด body ของ branch ที่ false + body ของ `{{#each}}` ออกก่อน scan → ตัวแปรใน false branch / loop-local `{{this}}` ไม่ถูกนับเป็น unset/missing. ตัวแปรใน true branch ยังถูกตรวจ.
- **Exports:** `resolveControlBlocks(html, variables, dataRow)` และ `stripControlBlocksForHealth(html, variables, dataRow)`.
