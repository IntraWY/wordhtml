...
describe("removeMergeFieldFromHtml", () => {
  ...
  it("removes the remaining filters {{name|currency|percent|comma|upper|lower}}", () => {
    const html =
      "<p>{{x|currency}} {{x|percent}} {{x|comma}} {{x|upper}} {{x|lower}}</p>";
    const result = removeMergeFieldFromHtml(html, "x");
    expect(result).toBe("<p>    </p>");
  });
