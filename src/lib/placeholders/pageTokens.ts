...
export interface PageTokenContext {
  ...
  now?: Date;
  /**
   * Document/export filename for the {filename} token. Extension is stripped.
   * Falls back to "document" when unknown.
   */
 ...
 *   {filename}        document filename without extension ("document" fallback)
 *
...
export function replacePageTokens(html: string, ctx: PageTokenContext): string {
  ...
  const fileBase =
    (ctx.fileName ?? "").replace(/\.[^.]+$/, "").trim() || "document";
...
export const PAGE_TOKEN_HELP: ReadonlyArray<{
  ...
  { token: "{filename}", description: "ชื่อไฟล์เอกสาร ไม่รวมนามสกุล" },
];
